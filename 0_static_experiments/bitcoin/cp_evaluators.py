...
def create_node_degree_mcp(model, data, calibration_indices, bins):
  ...
  alphas = []
  for y_probas, yt in zip(y_hat,y_true[y_true != -1]):
    y = yt.item()
    _alphas = get_nonconformity_measure_for_classification(y_probas)
    alphas.append(_alphas[y])
  ...
  cp = NodeDegreeMondrianConformalClassifier(alphas.cpu(), y.cpu(), node_degrees_bins[y_true != -1].cpu())
...
def create_node_degree_weighted_cp(model, data, calibration_indices):
  ...
  cp = NodeDegreeWeightedConformalClassifier(alphas.cpu(), y.cpu(), node_degrees[y_true != -1].cpu())
  return cp
...
def create_embedding_weighted_cp(model, data, calibration_indices):
  ...
  cp = EmbeddingDistanceWeightedConformalClassifier(alphas.cpu(), y.cpu(), embeddings[y_true != -1].cpu())
  return cp