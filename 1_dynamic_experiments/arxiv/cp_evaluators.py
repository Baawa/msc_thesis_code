...
class MCPWithResamplingEvaluator(CPEvaluator):
  def capture(self, model, graph: Graph):
    ...
    mcp = create_mcp(y_hat[calibration_indices], y_true[calibration_indices])

    confidence_intervals = get_confidence_intervals_mcp(mcp, y_hat[test_indices], self.confidence_level)
...
class NodeDegreeWeightedCPEvaluator(CPEvaluator):
  def capture(self, model, cp: NodeDegreeWeightedConformalClassifier, graph: Graph):
    ...
    confidence_intervals = get_confidence_intervals_node_degree_weighted(cp, y_hat, node_degrees, self.confidence_level)
...
class NodeDegreeWeightedCPWithResamplingEvaluator(CPEvaluator):
  def capture(self, model, graph: Graph):
    ...
    cp = create_node_degree_weighted_cp(y_hat[calibration_indices], y_true[calibration_indices], node_degrees)

    node_degrees = get_node_degrees(graph.data, test_indices)

    confidence_intervals = get_confidence_intervals_node_degree_weighted(cp, y_hat[test_indices], node_degrees, self.confidence_level)
...
class EmbeddingWeightedCPWithResamplingEvaluator(CPEvaluator):
  def capture(self, model, graph: Graph):
    ...
    confidence_intervals = get_confidence_intervals_embedding_weighted(cp, y_hat[test_indices], embeddings[test_indices], self.confidence_level)
...
def create_node_degree_mcp(y_hat, y_true, node_degrees_bins):
  ...
  cp = NodeDegreeMondrianConformalClassifier(alphas.cpu(), y.cpu(), node_degrees_bins)
...
def create_node_degree_weighted_cp(y_hat, y_true, node_degrees):
  ...
  cp = NodeDegreeWeightedConformalClassifier(alphas.cpu(), y.cpu(), node_degrees)
  return cp
